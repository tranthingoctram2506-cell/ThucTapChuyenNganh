        ...
        <div class="col-md-8 offset-md-2">
            ...
            <form action="{{ route('admin.product.update', ['product' => $phim->id]) }}" method="POST">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control" id="image" name="image" value="{{ old('image', $phim->image) }}" required>
                </div>
                <div class="form-group mb-3">
                    <label for="description">Mô Tả:</label>
                    <textarea class="form-control" id="description" name="description" rows="3" required>{{ old('description', $phim->description) }}</textarea>
                    @error('description') <span class="text-danger">{{ $message }}</span> @enderror
                </div>
                <div class="form-group mb-3">
                    <label for="content">Nội Dung:</label>
                    <textarea class="form-control" id="content" name="content" rows="5" required>{{ old('content', $phim->content) }}</textarea>
                    @error('content') <span class="text-danger">{{ $message }}</span> @enderror
                </div>
                <div class="form-group mb-3">
                    <label for="price">Giá:</label>
                    <input type="number" class="form-control" id="price" name="price" value="{{ old('price', $phim->price) }}" required>
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-control" id="category_id" name="category_id" required>
                        ...
                        @forelse($categories ?? [] as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $phim->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @empty
